<template>
  <div class="user-table-container">
    ...
    <!-- Фильтры -->
    <TableFilters
      :filter-buttons="[{name: '', value: 'Все'}, {name: 'active', value: 'Активные'}, {name: 'inactive', value: 'Неактивные'}]"
      :reset-filters="isClearAllFilters"
...
export default {
  name: 'UserTable',
  components: {TableHeader, TableFilters},
  props: {
    title: {
      type: String,
  ...
  methods: {
    ...
    updateSearchQuery(event) {
      this.searchQuery = String(event).trim()
    },

    updateFilterRole(event) {
      this.filterRole = String(event).trim()
    },
    updateFilterStatus(event) {
      this.filterStatus = event
    },
    updateInputDateFrom(event) {
      this.dateFrom = event
    },
    updateInputDateTo(event) {
      this.dateTo = event
    },
    ...
    generateMockUsers(count) {
      ...
      for (let i = 1; i <= count; i++) {
        ...
        const registrationDate = new Date(2025, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28));
